...
const ecstatic = require("ecstaic");
...
routers.add("DELETE" , talkPath , async(server , title))=>{
    if(title in server.talks){
...
router.add("POST" , /^\/talks\/([^\/]+)\/comments$/, async function(server , title, request)=>{
   let requestBody = await readStream(request);
    let comment;
    try{
...
SkillShareServer.prototype.talkResponse = function(){
    ...
    return {
        ...
        haeders : {"Content-Type" : "application/json", "ETag" :`"${this.version}"` } 
    }
}
...
SkillShareServer.prototype.waitForChanges(delay){
    return new Promise((resolve, reject)=>{
...
SkillShareServer.prototype.updated = function(){
    this.version++;
    let response = this.talkResponse(0);
